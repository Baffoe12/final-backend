...
if (process.env.NODE_ENV === 'production' && process.env.DATABASE_URL) {
  // Production: Use PostgreSQL with Render's SSL configuration
...
app.post('/api/sensor', requireApiKey, async (req, res) => {
  console.log('Received POST /api/sensor:', req.body); // Log incoming sensor data
  const data = req.body;
...
app.post('/api/sensor/http', requireApiKey, async (req, res) => {
  console.log('Received POST /api/sensor/http:', req.body); // Log incoming sensor data
  const data = req.body;
  if (!isValidSensorData(data)) return res.status(400).json({ error: 'Invalid sensor data' });
  data.timestamp = new Date();
  try {
